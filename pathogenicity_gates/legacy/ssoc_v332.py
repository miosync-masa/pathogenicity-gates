"""
SSOC v3.32 "Gates 1+2+3a+3b+CSG" — Module
...
v3.32: charge_sheet_no_hb_guard — ma in {'A','G'}(bur>0.5)でCSG無効化
  v3.31の→Ala guardをGlyに拡張。Glyも側鎖なし→H-bond capacity=0。
  軍師subtask_001i: Gly拡張はAla以上に物理的根拠が強い。
  subtask_002a: validate_v332.py でv3.31比Δρ>=0を確認。
v3.31: charge_sheet_ala_guard — →Ala変異(bur>0.5)でCSG無効化
  →Ala は荷電除去であり charge_sheet の H-bond network 前提に反する。
  subtask_001f: wrong_direction 73%(8/11) が →Ala変異、odds ratio 13倍。
v3.30: Charge sub-gate — charge_helix / charge_sheet / charge_coil
  Phase='charge' split by mutation site SS for SS-specific corrections:
    charge_helix: dq damped by helix dipole shielding (dq×h_nb = -0.322)
      → fold guard: disabled for α-rich/other (global helix dipole overwhelms)
    ...
    charge_coil: long-range Coulomb dominates (lr_coulomb = +0.731)
      → burial guard: disabled for surface (bur<0.5, solvent screens Coulomb)
...
# charge_sheet_ala_guard (v3.31)
CSG_SHEET_ALA_BUR_THRESHOLD = 0.5  # burial threshold for →Ala/Gly guard
# charge_sheet_no_hb_guard (v3.32): H-bond capacity=0の変異をguard
CSG_SHEET_NO_HB_RESIDUES = frozenset({'A', 'G'})  # Ala & Gly: no sidechain HB
...
def compute_charge_subgate(mut_ss, dq, helix_nf, beta_nf, n_chg_nb, lr_coulomb, bur, fold='α/β', ma=None):
    ...
    Splits charge phase by mutation site SS to apply SS-specific physics:
      helix: helix dipole shielding dampens charge effect
        → disabled for α-rich/other folds (global helix dipole overwhelms local correction)
      sheet: charge propagates only via H-bond network (sheet neighbors)
      coil: long-range Coulomb (8-20Å) dominates over local dq
        → disabled for surface (bur<0.5) where solvent screens Coulomb
    ...
    if mut_ss == 'H':
        # Fold gate: α-rich/other → global helix dipole dominates, local correction unreliable
        if fold in ('α-rich', 'other'):
            return 0.0, {'gate': 'helix_fold_guard', 'fold': fold}
        corr = CSG_HELIX_DQ * dq + CSG_HELIX_DQ_HNB * dq * helix_nf + CSG_HELIX_DQ2 * dq * dq
        sub = 'charge_helix'
    elif mut_ss == 'E':
        # charge_sheet_no_hb_guard: →Ala/Gly変異はCSGの前提(H-bond network)に反する
        if ma in CSG_SHEET_NO_HB_RESIDUES and bur > CSG_SHEET_ALA_BUR_THRESHOLD:
            return 0.0, {'gate': 'charge_sheet_no_hb_guard', 'ma': ma}
        corr = CSG_SHEET_DQ * dq + CSG_SHEET_DQ_ENB * dq * beta_nf + CSG_SHEET_DQ_NCH * dq * n_chg_nb
        sub = 'charge_sheet'
    else:
        # Burial gate: surface coil → solvent screens Coulomb, CSG unreliable
        if bur < 0.5:
            return 0.0, {'gate': 'coil_surface_guard', 'bur': bur}
...
def predict_mutation(backbone, atoms, all_heavy, pos, wa, ma, ss_map=None, pdb_features=None, pdb_path=None):
    ...
    # Compute fold class from PDB-level SS fractions
    _fH = pdb_features.get('frac_H', 0)
    _fE = pdb_features.get('frac_E', 0)
    if _fH > 0.4 and _fE < 0.15: _fold = 'α-rich'
    elif _fE > 0.4 and _fH < 0.15: _fold = 'β-rich'
    elif _fH > 0.2 and _fE > 0.2: _fold = 'α/β'
    else: _fold = 'other'
    csg_comp = 0.0
    ...
    if phase == 'charge' and not has_cofactor:
        ...
        csg_comp, csg_info = compute_charge_subgate(
            ss, dq_csg, helix_nf, beta_nf, n_chg_nb, lr_coulomb, bur, fold=_fold, ma=ma)
        ddg_pred += csg_comp
    ...
    return {'ddg':ddg_pred,'species':species,'bur':bur,'nc':nc,'pf':polar_frac,
            ...
            'csg_comp':csg_comp,'csg_gate':csg_info.get('gate','none'),'fold':_fold,
